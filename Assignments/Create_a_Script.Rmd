...
All.files <- All.files[c(1,3,5,7)] # I originally did this with the fastq files only, but needed the .gz to push to github and I couldn't find a way to not include them in the file list above for some reason that worked so this is a horrible solution but my fucntion is broken now anyway and I seriously don't have time to fix it and have already spent like 6 hours on this class today and many others through the week :/
...
Many explitives could be put here because I'm annoyed, but at some point my function broke and I can't really figure out why I keep getting 'Scale for 'y' is already present. Adding another scale for 'y', which will replace the existing scale' as an error when this was working *perfectly* yesterday
```{r}
plots <- list()
figure.generations <- function(Files, here.file.path) {
  for (i in length(Files)){
  png(here::here(here.file.path, "Quality_Plots.png"))
...
## Test function
```{r}
figure.generations(All.files, out.path)
plotQualityProfile(All.files)
```
